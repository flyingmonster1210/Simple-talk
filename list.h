...
struct Node_s {
    void *pItem;
    int prev, next;
    bool occupied;  //when occupied == 1, it is in a list
};

typedef struct {
    Node *pNode;    //this is the current pointer pointing to the current node
    int curr;
    int size;
    int end, begin;
} List;
...
void print_information_in_arrays();
...
// Adds item after the current item, and makes item the current item.
// If the current pointer is before the start of the pList, the item is added at the start. If
// the current pointer is beyond the end of the pList, the item is added at the end.
// Returns 0 on success, -1 on failure.
int List_add(List* pList, void* pItem);

// Adds item before the current item, and makes the new item the current one.
